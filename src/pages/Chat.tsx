...
const Chat: React.FC = () => {
  ...
  const handleArchiveMemory = async () => {
    ...
    const chatHistoryText = messages
      ...
      .map(m => `${m.role === 'user' ? '我' : (selectedCharacterCard.name)}: ${m.content}`)
      .join('\n\n');
    ...
    try {
      const resultStr = await invoke<string>('analyze_character_memory', {
        request: {
          ...
          currentRelation: selectedCharacterCard.fields?.relationMemory || '尚未确立关系。',
      ...
      setEditedRelation(analysis.relationMemory || '');
  ...
  return (
    <div className="agent-chat" style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#faf9f5' }}>
        ...
        {isAnalyzing ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                <div style={{ fontSize: '13px', fontWeight: 600, color: '#33312e', marginBottom: '6px' }}>更新后的关系记忆</div>
                <Input.TextArea 
                  value={editedRelation} 
                  onChange={(e) => setEditedRelation(e.target.value)} 
                  autoSize={{ minRows: 3, maxRows: 6 }}
                  placeholder="更新后的关系记忆内容..."
                  style={{ borderRadius: '6px', borderColor: '#eae6df' }}
                />
              </div>
      ...
      {/* Header (Only shown when chat has started) */}
      {hasMessages && (
          ...
          <div className="agent-chat__header-actions">
            {selectedCharacterCard && (
              <Tooltip title={isSessionArchived ? "当前会话的记忆已封存到角色卡" : "封存本场对话记忆到角色卡并归档"}>
                <Button
                  type="text"
                  disabled={isStreaming || isSessionArchived}
                  icon={<FileProtectOutlined />}
                  onClick={handleArchiveMemory}
                  style={{
                    color: isSessionArchived ? '#8c8882' : '#d97757',
                    fontWeight: 500,
                    fontSize: 13,
                    display: 'inline-flex',
                    alignItems: 'center',
